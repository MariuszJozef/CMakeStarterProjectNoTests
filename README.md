...
* However this solution file is *transient*: any modifications made from within VS will be overwritten upon subsequent invocation of CMake generator: `cmake -S . -B buildVS/x64/Msvc -G "Visual Studio 17 2022" -A x64`.
...
```
N.B.
* `<sourceDir>` is usually the current directory, where topmost CMakeLists.txt resides, since usually `cd <projectDir>`, therefore
```
    <sourceDir> ---> .
```
